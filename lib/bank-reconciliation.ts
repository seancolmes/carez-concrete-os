...
export async function analyzeBankTransactions(supabase:any,companyId:string,{limit=750}:{limit?:number}={}){
  ...
  const [customersR,customerPaymentsR,invoicesR,vendorsR,vendorPaymentsR,billsR,expensesR,payrollR,taxesR,rulesR,overheadR,allTxR]=await Promise.all([
    ...
    supabase.from('overhead_items').select('id,category,name,business_use_percent,amount').eq('company_id',companyId).eq('active',true),
  ...
  const customerNames=new Map<string,string>((customersR.data||[]).map((x:any)=>[String(x.id),String(x.name||'')] as [string,string]));
  const vendorNames=new Map<string,string>((vendorsR.data||[]).map((x:any)=>[String(x.id),String(x.name||'')] as [string,string]));
  ...
  for(const t of txs||[]){
    ...
    const explicitBankFee=/(^| )(fee|overdraft|nsf)( |$)|service charge/.test(labelNorm);
    if(cash<0&&bankFeeOH&&explicitBankFee){addCandidate(map,{company_id:companyId,bank_transaction_id:t.id,candidate_type:'company_expense',entity_type:'heuristic',confidence:92,reason:'Transaction description explicitly indicates a bank fee.',proposed_action:'Record Banking overhead expense',metadata:{overhead_item_id:bankFeeOH.id,expense_category:bankFeeOH.category,business_use_percent:Number(bankFeeOH.business_use_percent||100)},status:'active'});}
    ...
    const qbPlan=Number(qbOH?.amount||0);if(cash<0&&qbOH&&qbPlan>0&&/quickbooks/.test(labelNorm)&&Math.abs(amt-qbPlan)<=Math.max(25,qbPlan*.35)){addCandidate(map,{company_id:companyId,bank_transaction_id:t.id,candidate_type:'company_expense',entity_type:'heuristic',confidence:91,reason:'Merchant and amount resemble the planned QuickBooks subscription.',proposed_action:'Record QuickBooks overhead expense',metadata:{overhead_item_id:qbOH.id,expense_category:qbOH.category,business_use_percent:Number(qbOH.business_use_percent||100)},status:'active'});}
  ...
  const possibleTransferPairs:{a:any;b:any}[]=[];const partnerCounts=new Map<string,number>();
  for(let i=0;i<allTx.length;i++)for(let j=i+1;j<allTx.length;j++){
    const a=allTx[i],b=allTx[j];if(a.account_id===b.account_id||Number(a.cash_amount)*Number(b.cash_amount)>=0||!sameMoney(Math.abs(Number(a.cash_amount)),Math.abs(Number(b.cash_amount)))||days(a.transaction_date,b.transaction_date)>1)continue;
    const transferHint=String(a.category_primary||'').toUpperCase().includes('TRANSFER')||String(b.category_primary||'').toUpperCase().includes('TRANSFER')||/transfer/.test(norm(`${a.name} ${b.name}`));if(!transferHint)continue;
    possibleTransferPairs.push({a,b});partnerCounts.set(a.id,(partnerCounts.get(a.id)||0)+1);partnerCounts.set(b.id,(partnerCounts.get(b.id)||0)+1);
  }
  for(const {a,b} of possibleTransferPairs){
    if((partnerCounts.get(a.id)||0)!==1||(partnerCounts.get(b.id)||0)!==1)continue;
    addCandidate(map,{company_id:companyId,bank_transaction_id:a.id,candidate_type:'internal_transfer',entity_type:'bank_transaction',entity_id:b.id,confidence:98,reason:'Unique equal-and-opposite transfer between two Carez bank accounts.',proposed_action:'Match internal transfer',metadata:{other_transaction_id:b.id,other_name:b.name,amount:Math.abs(Number(a.cash_amount))},status:'active'});
    addCandidate(map,{company_id:companyId,bank_transaction_id:b.id,candidate_type:'internal_transfer',entity_type:'bank_transaction',entity_id:a.id,confidence:98,reason:'Unique equal-and-opposite transfer between two Carez bank accounts.',proposed_action:'Match internal transfer',metadata:{other_transaction_id:a.id,other_name:a.name,amount:Math.abs(Number(b.cash_amount))},status:'active'});
  }
  ...
  for(const t of txs||[]){const list=(byTx.get(t.id)||[]).sort((a,b)=>b.confidence-a.confidence);const top=list[0],second=list[1];if(!top)continue;
    ...
    if(top.candidate_type==='internal_transfer'&&top.confidence>=98&&(!second||second.confidence<98)&&top.entity_id){try{const {error}=await supabase.rpc('reconcile_bank_as_transfer',{p_bank_transaction_id:t.id,p_other_bank_transaction_id:top.entity_id});if(!error){autoMatched++;continue;}}catch{}}
