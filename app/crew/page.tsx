...
export default async function CrewPage(){
 ...
 const active=(crew||[]).filter(c=>c.active).length;const onCall=(crew||[]).filter(c=>c.active&&c.availability_type==='on_call').length;const w2=(crew||[]).filter(c=>c.active&&c.worker_type==='employee').length;
 return <AppShell userName={profile.full_name||user.email||'Owner'}><div className="contractor-page">
  <div className="command-hero"><div><div className="section-kicker">CREW CONTROL</div><h1>Crew</h1><p>Workers, labor rates, field access and skills. Production quantities come from work packages, not daily math assigned to employees.</p></div><div className="command-actions"><Link className="button secondary" href="/production/work-packages">Work Packages</Link><Link className="button secondary" href="/crew/access">Employee Access</Link></div></div>
  <div className="command-grid"><div className="command-card"><div className="command-label">Active Crew</div><div className="command-value">{active}</div></div><div className="command-card"><div className="command-label">W-2 Employees</div><div className="command-value">{w2}</div></div><div className="command-card"><div className="command-label">On-Call Bench</div><div className="command-value">{onCall}</div></div><div className="command-card good"><div className="command-label">Field Data Rule</div><div className="command-value">Simple</div><div className="command-help">Employees identify work and confirm completion. Carez owns the quantities and math.</div></div></div>

  <div className="card section"><div className="title">Add Crew Member</div><p className="subtitle">Employees should never need cost codes or production calculations to submit clean field time.</p>
