...
export default async function CrewPage(){
 ...
 const active=(crew||[]).filter(c=>c.active).length;const onCall=(crew||[]).filter(c=>c.active&&c.availability_type==='on_call').length;const w2=(crew||[]).filter(c=>c.active&&c.worker_type==='employee').length;const reporters=(crew||[]).filter(c=>c.active&&c.can_report_production).length;
 return <AppShell userName={profile.full_name||user.email||'Owner'}><div className="contractor-page">
  <div className="command-hero"><div><div className="section-kicker">CREW CONTROL</div><h1>Crew</h1><p>Workers, labor rates, field access and production responsibility in one place.</p></div><div className="command-actions"><Link className="button secondary" href="/production">Production Control</Link><Link className="button secondary" href="/crew/access">Employee Access</Link></div></div>
  <div className="command-grid"><div className="command-card"><div className="command-label">Active Crew</div><div className="command-value">{active}</div></div><div className="command-card"><div className="command-label">W-2 Employees</div><div className="command-value">{w2}</div></div><div className="command-card"><div className="command-label">On-Call Bench</div><div className="command-value">{onCall}</div></div><div className={`command-card ${reporters?'good':''}`}><div className="command-label">Crew Reporters</div><div className="command-value">{reporters}</div><div className="command-help">Employees trusted to enter the crew's completed quantity.</div></div></div>

  <div className="card section"><div className="title">Add Crew Member</div><p className="subtitle">Regular employees only identify the work they are doing. Only a designated lead should report the crew's production quantity.</p>
   <form action={createCrewMember} className="form">
    ...
    <div className="grid grid2"><label className="field"><span>Default L&I Class</span><select name="default_risk_class_code" defaultValue="0217-01"><option value="">None / review required</option>{(riskClasses||[]).map((r:any)=><option key={r.code} value={r.code}>{r.code} — {r.name}</option>)}</select></label><label className="field"><span>Start Date</span><input name="start_date" type="date" defaultValue={today()}/></label></div>
    <label className="field"><span>Phone</span><input name="phone" inputMode="tel" placeholder="253-555-0123"/></label>
    <label className="meta"><input type="checkbox" name="can_report_production"/> Crew lead / foreman — can report the crew's completed quantity</label>
  ...
  <div className="grid section">{(crew||[]).map(c=>{const history=rateMap.get(c.id)||[];return <div className="card" key={c.id}>
   <div className="row"><div><div className="title">{c.name}</div><div className="meta">{c.role} · {c.worker_type==='employee'?'W-2 employee':c.worker_type} · {c.availability_type.replace('_',' ')}</div></div><div className="action-row"><span className={`status ${c.can_report_production?'completed':''}`}>{c.can_report_production?'Crew Reporter':'Time Only'}</span><span className="status">{c.active?'Active':'Inactive'}</span></div></div>
   ...
   <details className="section"><summary className="title" style={{cursor:'pointer'}}>Edit Worker</summary><form action={updateCrewMember} className="form" style={{marginTop:12}}><input type="hidden" name="id" value={c.id}/>
    ...
    <div className="grid grid2"><label className="field"><span>Default L&I</span><select name="default_risk_class_code" defaultValue={c.default_risk_class_code||''}><option value="">None / review required</option>{(riskClasses||[]).map((r:any)=><option key={r.code} value={r.code}>{r.code} — {r.name}</option>)}</select></label><label className="field"><span>Start Date</span><input name="start_date" type="date" defaultValue={c.start_date||''}/></label></div>
    <label className="field"><span>Phone</span><input name="phone" defaultValue={c.phone||''}/></label>
    <label className="meta"><input type="checkbox" name="can_report_production" defaultChecked={Boolean(c.can_report_production)}/> Crew lead / foreman — can report completed crew quantity</label>
