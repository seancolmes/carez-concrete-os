...
import {submitProposalResponse} from './actions';
...
export default async function PublicProposalPage({params,searchParams}:{params:Promise<{token:string}>,searchParams:Promise<{preview?:string}>}){
 ...
 const supportsResponses=Boolean(presentation.id);
 const active=!acceptance&&presentation.status!=='declined'&&supportsResponses;
 ...
 return <main className={styles.page}>
  ...
  <div className={styles.document}>
   ...
   {acceptance?<section className={styles.accepted}><span>Accepted</span><h2>Thank you, {acceptance.accepted_name}.</h2><p>{proposalNumber} was accepted on {date(acceptance.accepted_at)}. This records your historical response to this proposal. Any internal Award and commercial baseline are handled separately by Carez.</p>{project?.job_number&&<strong>Carez Job {project.job_number}</strong>}</section>:presentation.status==='declined'?<section className={styles.declined}><h2>Proposal response recorded</h2><p>If circumstances change, contact {companyName} and we can reopen the conversation or issue a current proposal.</p></section>:<section className={styles.actionSection}>
    <div className={styles.sectionLabel}>Next Step</div><h2>Questions or changes?</h2><p>Review the scope and price in {proposalNumber}. Contact {companyName} to discuss moving forward; online acceptance is not available for this proposal.</p>
