  ...
  return <AppShell userName={profile.full_name||user.email||'Owner'}>
    <div className="mx-auto flex w-full max-w-screen-2xl flex-col gap-6">
      <header className="carez-page-heading flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2"><Link className={buttonVariants({variant:'outline',size:'sm'})} href="/estimates"><FileText/>Estimates</Link><Link className={buttonVariants({variant:'outline',size:'sm'})} href="/estimates/audit"><ShieldCheck/>Review</Link></div>
      </header>
      ...
      <section className="space-y-4">
        <div><p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Customer pipeline</p><h2 className="mt-1 text-lg font-semibold">Ready & in market</h2><p className="mt-1 text-sm text-muted-foreground">Prepare, send, follow up, and respond from the current authoritative proposal state.</p></div>
        {ready.length+market.length===0?<Empty className="min-h-56 border bg-muted/20"><EmptyHeader><EmptyMedia variant="icon"><FileText/></EmptyMedia><EmptyTitle>No proposal is waiting right now</EmptyTitle><EmptyDescription>Finish an estimate, complete Review, then move the revision into proposal preparation.</EmptyDescription></EmptyHeader><EmptyContent><Link className={buttonVariants()} href="/estimates">Open estimates</Link></EmptyContent></Empty>:<div className="grid gap-3 lg:grid-cols-2">{[...ready,...market].map((row:any)=><ProposalCard key={row.e.id} row={row}/>)}</div>}
      </section>

      {won.length>0?<section className="space-y-4"><div><p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Won</p><h2 className="mt-1 text-lg font-semibold">Accepted proposals</h2><p className="mt-1 text-sm text-muted-foreground">Customer acceptance is separate from internal Award. Older accepted proposals may need review; those without reliable frozen scope and cost evidence remain on hold.</p></div><div className="grid gap-3 lg:grid-cols-2">{won.map((row:any)=><ProposalCard key={row.e.id} row={row}/>)}</div></section>:null}
