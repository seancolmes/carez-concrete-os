...
function Metric({label,value,help,tone='default'}:{label:string;value:string;help?:string;tone?:'default'|'success'|'warning'|'danger'}){
  return <Card className={cn('gap-2 py-4 shadow-none',tone==='warning'&&'border-amber-500/30',tone==='danger'&&'border-destructive/25')}><CardHeader className="gap-1 px-4"><CardDescription className="text-xs font-medium">{label}</CardDescription><CardTitle className={cn('font-mono text-xl font-semibold tracking-tight tabular-nums',tone==='success'&&'text-success',tone==='warning'&&'text-amber-700',tone==='danger'&&'text-destructive')}>{value}</CardTitle></CardHeader>{help?<CardContent className="px-4 text-xs leading-5 text-muted-foreground">{help}</CardContent>:null}</Card>;
}
...
export default async function BillingPage(){
  ...
  return <AppShell userName={p.full_name||user.email||'Owner'}>
    <div className="carez-page">
      <header className="carez-page-header"><div><p className="carez-kicker">Finance</p><h1 className="carez-page-title">Billing</h1><p className="carez-page-description">Bill the customer, see what they still owe, collect the money, and keep retainage from getting forgotten.</p></div></header>
      ...
      <section className="carez-section"><div className="carez-section-header"><div><p className="carez-kicker">Workflow</p><h2 className="carez-section-title">From work performed to money in the bank</h2></div></div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{workflow.map(({href,label,copy,Icon},index)=><Link href={href} key={href}><Card className="h-full gap-3 py-4 shadow-none transition-colors hover:bg-muted/40"><CardHeader className="grid grid-cols-[36px_minmax(0,1fr)] items-start gap-3 px-4"><span className="flex size-9 items-center justify-center rounded-lg bg-accent text-primary"><Icon className="size-4"/></span><div><p className="mb-1 text-[10px] font-medium text-muted-foreground">STEP {index+1}</p><CardTitle className="text-sm">{label}</CardTitle><CardDescription className="mt-1 text-xs leading-5">{copy}</CardDescription></div></CardHeader></Card></Link>)}</div></section>

      <section className="carez-section">
        <div className="carez-section-header"><div><p className="carez-kicker">Jobs</p><h2 className="carez-section-title">Where the money stands</h2><p className="carez-section-description">Authorized contract value, billing progress, receivables, and cash collected by job.</p></div></div>
        {(billing||[]).length===0?<Empty className="min-h-56 border bg-muted/20"><EmptyHeader><EmptyMedia variant="icon"><ReceiptText/></EmptyMedia><EmptyTitle>No project billing yet</EmptyTitle><EmptyDescription>Project billing records will appear here once invoices are issued against awarded work.</EmptyDescription></EmptyHeader></Empty>:
          <Card className="py-0 shadow-none"><Table><TableHeader><TableRow className="bg-muted/30 hover:bg-muted/30"><TableHead>Job</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Authorized</TableHead><TableHead className="text-right">Not billed</TableHead><TableHead className="text-right">Billed</TableHead><TableHead className="text-right">Still owed</TableHead><TableHead className="text-right">Collected</TableHead></TableRow></TableHeader><TableBody>{(billing||[]).map((row:any)=>{const overdue=num(row.overdue_ar),outstanding=num(row.outstanding_ar);return <TableRow key={row.project_id}><TableCell><Link href={`/projects/${row.project_id}`} className="font-medium hover:text-primary">{row.job_number} — {row.name}</Link></TableCell><TableCell><Badge variant={overdue>0?'destructive':outstanding>0?'default':'secondary'} className={!outstanding?'bg-success/10 text-success':''}>{overdue>0?'Payment late':outstanding>0?'Customer owes':'Current'}</Badge></TableCell><TableCell className="carez-data-number text-right">{money(row.authorized_contract)}</TableCell><TableCell className="carez-data-number text-right">{money(row.unbilled_contract)}</TableCell><TableCell className="carez-data-number text-right">{money(row.billed_contract)}</TableCell><TableCell className={cn('carez-data-number text-right font-medium',overdue>0&&'text-destructive',outstanding>0&&!overdue&&'text-amber-700')}>{money(row.outstanding_ar)}</TableCell><TableCell className="carez-data-number text-right text-success">{money(row.cash_collected)}</TableCell></TableRow>})}</TableBody></Table></Card>}
      </section>
    </div>
  </AppShell>;
}
