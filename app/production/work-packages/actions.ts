...
export async function addWorkPackageOperation(fd:FormData){
 ...
 const {error}=await supabase.from('work_package_operations').insert({company_id:companyId,work_package_id:packageId,production_task_id:taskId,field_label:String(fd.get('field_label')||'').trim()||null,planned_quantity:qty,unit:task.production_unit,budgeted_man_hours:budget,baseline_man_hours_per_unit:baseline,baseline_source:String(fd.get('baseline_source')||'manual'),measurement_method:String(fd.get('measurement_method')||'completion'),notes:String(fd.get('notes')||'').trim()||null,status:'planned'});if(error)throw new Error(error.message);
 revalidatePath('/production/work-packages');revalidatePath('/schedule');revalidatePath('/production');
}

export async function completeWorkPackageOperation(fd:FormData){
 const id=String(fd.get('operation_id')||'');if(!id)return;const {supabase,user,companyId}=await ctx();const {data:op,error}=await supabase.from('work_package_operations').update({status:'completed',completed_at:new Date().toISOString(),completed_by_profile_id:user.id,completed_by_crew_member_id:null,completion_source:'owner_confirmation',updated_at:new Date().toISOString()}).eq('id',id).eq('company_id',companyId).select('work_package_id').single();if(error)throw new Error(error.message);if(op){const {count}=await supabase.from('work_package_operations').select('id',{count:'exact',head:true}).eq('work_package_id',op.work_package_id).eq('company_id',companyId).not('status','in','("completed","cancelled")');await supabase.from('work_packages').update({status:(count||0)===0?'completed':'active',updated_at:new Date().toISOString()}).eq('id',op.work_package_id).eq('company_id',companyId);}revalidatePath('/production/work-packages');revalidatePath('/production');revalidatePath('/employee');
}

export async function reopenWorkPackageOperation(fd:FormData){
 const id=String(fd.get('operation_id')||'');if(!id)return;const {supabase,companyId}=await ctx();const {data:op,error}=await supabase.from('work_package_operations').update({status:'in_progress',completed_at:null,completed_by_profile_id:null,completed_by_crew_member_id:null,completion_source:null,updated_at:new Date().toISOString()}).eq('id',id).eq('company_id',companyId).select('work_package_id').single();if(error)throw new Error(error.message);if(op)await supabase.from('work_packages').update({status:'active',updated_at:new Date().toISOString()}).eq('id',op.work_package_id).eq('company_id',companyId);revalidatePath('/production/work-packages');revalidatePath('/production');revalidatePath('/employee');
}
