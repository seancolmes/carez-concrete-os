...
export default async function ChangeOrdersPage(){
  ...
  return <AppShell userName={profile?.full_name||user.email||'Owner'}><div className="mx-auto flex w-full max-w-screen-2xl flex-col gap-6">
    ...
    <div className="carez-summary-ledger grid grid-cols-2 gap-px xl:grid-cols-4">
      <Card size="sm" className="rounded-none border-x-0 bg-transparent shadow-none"><CardContent className="space-y-1"><div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Active Change Orders</div><div className="font-mono text-2xl font-semibold tabular-nums">{activeCOs}</div></CardContent></Card>
      <Card size="sm" className="rounded-none border-x-0 bg-transparent shadow-none"><CardContent className={submitted?'space-y-1 text-warning':'space-y-1 text-success'}><div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Awaiting Approval</div><div className="font-mono text-2xl font-semibold tabular-nums">{submitted}</div></CardContent></Card>
      <Card size="sm" className="rounded-none border-x-0 bg-transparent shadow-none"><CardContent className="space-y-1"><div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Approved Contract Changes</div><div className="font-mono text-2xl font-semibold tabular-nums">{money(approvedValue)}</div></CardContent></Card>
      <Card size="sm" className={pendingExposure>0?'rounded-none border-x-0 border-destructive/30 bg-transparent shadow-none':'rounded-none border-x-0 bg-transparent shadow-none'}><CardContent className={pendingExposure>0?'space-y-1 text-destructive':'space-y-1 text-success'}><div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Unapproved Field Exposure</div><div className="font-mono text-2xl font-semibold tabular-nums">{money(pendingExposure)}</div></CardContent></Card>
    </div>
