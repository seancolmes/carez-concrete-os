...
export default async function EstimateDetail({params}:{params:Promise<{estimateId:string}>}){
  ...
  return <AppShell userName={p.full_name||user.email||'Owner'}><div className="mx-auto flex w-full max-w-screen-2xl flex-col gap-6">
    ...
    <section className="grid grid-cols-1 divide-y divide-border border-y border-border md:grid-cols-4 md:divide-x md:divide-y-0" aria-label="Commercial estimator ledger">
      ...
      <LedgerMetric label="P1.4 Release Readiness" value={readinessStatus} help={`${readiness.blocker_count} blocker(s) · ${readiness.warning_count} warning(s). Evaluated by the database.`} tone={readiness.release_state==='blocked'?'error':readiness.release_state==='review'?'warning':readiness.release_state==='release_ready'?'success':'default'}/>
    </section>
    ...
    <div id="pricing-coverage" className="scroll-mt-6"><PricingCoverage estimateId={e.id} measurements={measurements||[]} outputs={estimateOutputs} quoteSets={quoteSets||[]} quotes={quotes} quoteLines={quoteLines} locked={locked} today={today}/></div>

    <div id="labor-review" className="scroll-mt-6"><LaborReview estimateId={e.id} measurements={measurements||[]} outputs={estimateOutputs} laborProfiles={laborProfiles||[]} locked={locked}/></div>
