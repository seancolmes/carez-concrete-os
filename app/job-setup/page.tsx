...
export default async function JobSetupPage(){
  ...
  return <AppShell userName={profile.full_name||user.email||'Owner'}><div className="mx-auto flex w-full max-w-screen-2xl flex-col gap-6">
    ...
    <div className="carez-summary-ledger grid grid-cols-2 gap-px xl:grid-cols-5">
      ...
      <Card size="sm" className="rounded-none border-x-0 bg-transparent shadow-none"><CardContent className={cn('h-full space-y-1',generated.length?'text-success':'')}><div className="text-xs font-medium text-muted-foreground">Auto Field Plans</div><div className="break-words font-mono text-2xl font-semibold tabular-nums">{generated.length}</div><div className="text-xs text-muted-foreground">Accepted takeoffs already connected to physical field work.</div></CardContent></Card>
      <Card size="sm" className="rounded-none border-x-0 bg-transparent shadow-none"><CardContent className={cn('h-full space-y-1',ready.length?'text-success':'')}><div className="text-xs font-medium text-muted-foreground">Setup Clear</div><div className="break-words font-mono text-2xl font-semibold tabular-nums">{ready.length}</div><div className="text-xs text-muted-foreground">Agreement, billing and pre-start money are clear. Operation readiness still controls actual start permission.</div></CardContent></Card>
      <Card size="sm" className="rounded-none border-x-0 bg-transparent shadow-none"><CardContent className={cn('h-full space-y-1',hold.length?'text-destructive':'')}><div className="text-xs font-medium text-muted-foreground">Setup Holds</div><div className="break-words font-mono text-2xl font-semibold tabular-nums">{hold.length}</div><div className="text-xs text-muted-foreground">Something must be handled before scheduling crew work.</div></CardContent></Card>
      <Card size="sm" className="rounded-none border-x-0 bg-transparent shadow-none"><CardContent className={cn('h-full space-y-1',prestart>0?'text-warning':'')}><div className="text-xs font-medium text-muted-foreground">Pre-Start Money Open</div><div className="break-words font-mono text-2xl font-semibold tabular-nums">{money(Math.max(0,prestart))}</div><div className="text-xs text-muted-foreground">Required payments not yet cleared.</div></CardContent></Card>
    </div>
