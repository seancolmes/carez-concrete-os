  ...
  return <AppShell userName={profile.full_name||user.email||'Owner'}>
    <div className="mx-auto flex w-full max-w-screen-2xl flex-col gap-6">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div><p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Assembly &amp; resource engine</p><h1 className="mt-1 text-2xl font-semibold tracking-tight">Assemblies</h1><p className="mt-1 max-w-4xl text-sm text-muted-foreground">Company-owned concrete recipes that convert measured scope into labor, material, equipment, and production quantities.</p></div>
      ...
      <section className="space-y-4">
        <div><p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Published library</p><h2 className="mt-1 text-lg font-semibold">Concrete assemblies</h2><p className="mt-1 text-sm text-muted-foreground">{publishedAssemblies.length} published assembly{publishedAssemblies.length===1?' is':' assemblies are'} available in Takeoff.</p></div>
