...
export default async function ProjectsPage(){
  ...
  const [{data:projects},{data:timecards},{data:company}]=await Promise.all([
    supabase.from('projects').select('*,customers(name)').order('created_at',{ascending:false}),
    supabase.from('timecards').select('project_id,hours,hourly_rate'),
    profile?.company_id?supabase.from('companies').select('labor_burden_percent').eq('id',profile.company_id).maybeSingle():Promise.resolve({data:null})
  ]);
  const burden=num(company?.labor_burden_percent);
  const laborByProject=new Map<string,{hours:number,cost:number}>();
  for(const t of timecards||[]){const current=laborByProject.get(t.project_id)||{hours:0,cost:0};const hours=num(t.hours);const base=hours*num(t.hourly_rate);current.hours+=hours;current.cost+=base*(1+burden/100);laborByProject.set(t.project_id,current);}
  return <AppShell userName={profile?.full_name||user.email||'Owner'}>
    <h1 className="page-title">Projects</h1><p className="subtitle">Contract, production and real labor job cost.</p>
    ...
    <div className="alert warn section"><strong>Labor burden:</strong> {burden.toFixed(2)}%. Change this under Settings. Actual labor cost = hours × base hourly rate × (1 + burden%).</div>
    <div className="grid section">{(projects||[]).length===0&&<div className="card meta">No projects loaded yet.</div>}{(projects||[]).map(p=>{const adjusted=num(p.contract_value)+num(p.approved_changes)-num(p.backcharges);const actual=laborByProject.get(p.id)||{hours:0,cost:0};const estHours=num(p.estimated_labor_hours);const estCost=num(p.estimated_labor_cost);const remainingHours=estHours-actual.hours;const remainingCost=estCost-actual.cost;return <div className="card" key={p.id}>
      ...
      <div className="grid grid4 section"><div><div className="label">Adjusted Contract</div><strong>{money(adjusted)}</strong></div><div><div className="label">Est. Labor</div><strong>{money(estCost)}</strong><div className="meta">{estHours.toFixed(1)} hr</div></div><div><div className="label">Actual Labor</div><strong>{money(actual.cost)}</strong><div className="meta">{actual.hours.toFixed(1)} hr</div></div><div><div className="label">Labor Remaining</div><strong>{money(remainingCost)}</strong><div className="meta">{remainingHours.toFixed(1)} hr</div></div></div>
