...
export async function createProjectCost(fd:FormData){
 ...
 const catalog=String(fd.get('catalog_item_id')||'')||null;
 await supabase.from('project_costs').insert({company_id:companyId,project_id,cost_code_id,catalog_item_id:catalog,cost_date:String(fd.get('cost_date')||new Date().toISOString().slice(0,10)),description,vendor_name:String(fd.get('vendor_name')||'').trim()||null,quantity,unit:String(fd.get('unit')||'LS'),unit_cost:unitCost,subtotal,sales_tax:tax,total_cost:total,source_type:String(fd.get('source_type')||'manual'),reference_number:String(fd.get('reference_number')||'').trim()||null,notes:String(fd.get('notes')||'').trim()||null,created_by:user.id});
