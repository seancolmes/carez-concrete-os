...
export default async function Dashboard(){
  ...
  const [{data:profile},{data:projects},{data:leads},{data:billing}]=await Promise.all([
  ...
  return <AppShell userName={name}>
    ...
    {overdueAR>0&&<div className="alert danger"><strong>Collections attention.</strong> {money(overdueAR)} is currently overdue.</div>}
    ...
    <div className="card section"><div className="title">Quick Actions</div><div className="nav" style={{marginTop:12}}><Link href="/leads">New Lead</Link><Link href="/projects">Projects</Link><Link href="/billing">Billing</Link><Link href="/change-orders">Change Orders</Link><Link href="/field">Field</Link><Link href="/costs">Job Costs</Link></div></div>
