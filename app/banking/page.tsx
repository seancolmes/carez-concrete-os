...
export default async function BankingPage(){
  ...
  const s:any=summary||{},r:any=reconciliation||{},c:any=cash||{},configured=plaidConfigured(),providerEnabled=providerMutationAllowed();
  ...
  return <AppShell userName={profile.full_name||user.email||'Owner'}><div className="mx-auto flex w-full max-w-screen-2xl flex-col gap-6">
    <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between"><header className="carez-page-heading"><div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Finance</div><h1 className="mt-1 text-2xl font-semibold tracking-tight">Banking</h1><p className="mt-1 max-w-3xl text-sm text-muted-foreground">Connected bank balances, transaction feed and reconciliation status in Carez.</p></header><div className="flex flex-wrap gap-2"><Link className={buttonVariants()} href="/banking/reconcile">Reconcile Transactions{needs>0?` (${needs})`:''}</Link><PlaidConnectButton configured={configured} providerEnabled={providerEnabled}/>{Number(s.active_connections||0)>0&&<RefreshBankButton providerEnabled={providerEnabled}/>}<Link className={buttonVariants({variant:'outline'})} href="/cashflow">Cashflow</Link></div></div>
    {!providerEnabled&&<p role="status" className="text-sm text-muted-foreground">Bank connections and refresh are disabled in this environment.</p>}

    {providerEnabled&&!configured&&<div className="rounded-lg border border-warning/30 bg-warning/10 px-4 py-3 text-sm text-warning"><strong>Plaid setup is incomplete.</strong> Configure PLAID_CLIENT_ID, PLAID_SECRET, and an explicit PLAID_ENV on the server before connecting a bank.</div>}
