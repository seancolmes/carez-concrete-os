...
import {resolveRoleMeasurementStartState,type ConditionRoleMeasurementRequest as SpecialistConditionRoleMeasurementRequest} from '@/lib/takeoff/specialistWorkstation';
import styles from './TakeoffDrawingCanvas.module.css';

type Tool='select'|'pan'|'calibrate'|'scaleRegion'|'draw'|'cutout'|'edit';
export type ConditionRoleMeasurementRequest=SpecialistConditionRoleMeasurementRequest;
...
export function TakeoffDrawingCanvas(props:TakeoffDrawingCanvasProps){
  ...
  const legacyChrome=Boolean(props.legacyChrome);
  const conditionMeasurementIdSet=useMemo(()=>new Set(props.conditionMeasurementIds||[]),[props.conditionMeasurementIds]);
  const router=useRouter();
  const openConditions=useCallback(()=>{if(!locked)props.onOpenConditions?.();},[locked,props.onOpenConditions]);
  ...
  const [pageNumber,setPageNumber]=useState(()=>Number(initialSheets.find((sheet:any)=>String(sheet.id)===String(props.activeSheetId||''))?.page_number||initialSheets[0]?.page_number||1));
  ...
  const selectedMeasurementId=props.selectedMeasurementId??null;
  const setSelectedMeasurementId=useCallback((id:string|null)=>props.onSelectedMeasurementChange(id),[props.onSelectedMeasurementChange]);
  ...
  const startedRoleRequestIdRef=useRef<string|null>(null);
  ...
  const hasValidCurrentScale=currentScaleRegions.some(region=>Number(region.calibration?.ft_per_pdf_unit)>0)||Number(currentSheet?.calibration?.ft_per_pdf_unit)>0;
  ...
  useEffect(()=>{if(!selectedMeasurement)return;const sheet=initialSheets.find((s:any)=>s.id===selectedMeasurement.sheet_id);if(sheet&&Number(sheet.page_number)!==pageNumber){setPageNumber(Number(sheet.page_number));props.onActiveSheetChange(String(sheet.id));}},[selectedMeasurementId,selectedMeasurement?.sheet_id]);
  useEffect(()=>{const requested=props.activeSheetId?initialSheets.find((sheet:any)=>String(sheet.id)===String(props.activeSheetId)):null;if(requested&&Number(requested.page_number)!==pageNumber){setPageNumber(Number(requested.page_number));setDraftPoints([]);setDraftScaleRegionId(null);setCalibrationPoints([]);setScaleRegionPoints([]);setEditGeometry(null);editOriginalRef.current=null;setHoverPoint(null);startedRoleRequestIdRef.current=null;}},[props.activeSheetId,pageNumber,initialSheets]);
  ...
  function beginRoleMeasurementRequest(request:ConditionRoleMeasurementRequest,force=false){
    if(locked||!conditionAuthoringActive)return;
    const version:any=versionMap.get(String(request.assemblyVersionId||''));
    if(!version){setMessage('The Concrete Condition takeoff recipe is unavailable. Refresh and try again.');return;}
    if(version.assembly_id!==selectedAssemblyId){setSelectedAssemblyId(version.assembly_id);return;}
    const assembly:any=assemblyMap.get(version.assembly_id);
    const start=resolveRoleMeasurementStartState(request,String(assembly?.primary_measurement||''),hasValidCurrentScale);
    if(start.state==='scale-required'){
      startedRoleRequestIdRef.current=null;
      setConditionDrawActive(false);
      setCalibrationPoints([]);
      setInspectorOpen(true);
      setInspectorTab('properties');
      setTool('calibrate');
      setMessage('Scale required for '+request.roleLabel+'. Confirm a detected scale or calibrate this sheet; the Condition role will remain pending.');
      return;
    }
    if(!force&&startedRoleRequestIdRef.current===request.requestId)return;
    startedRoleRequestIdRef.current=request.requestId;
    setConditionDrawActive(true);
    setSelectedMeasurementId(null);
    setEditGeometry(null);
    editOriginalRef.current=null;
    setObjectName(request.objectName);
    setDraftPoints([]);
    setDraftScaleRegionId(null);
    setHoverPoint(null);
    setTool('draw');
  ...
  useEffect(()=>{
    const request=props.roleMeasurementRequest;
    if(!request){startedRoleRequestIdRef.current=null;return;}
    beginRoleMeasurementRequest(request);
  },[props.roleMeasurementRequest?.requestId,selectedAssemblyId,selectedVersion?.id,currentSheet?.id,hasValidCurrentScale,locked]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const finishDraft=useCallback(async()=>{
    ...
    try{
      ...
      props.onMeasurementCommitted(String(result.id));
      if(conditionAuthoringActive&&props.roleMeasurementRequest){props.onRoleMeasurementRequestConsumed(props.roleMeasurementRequest.requestId);startedRoleRequestIdRef.current=null;}
  ...
  useEffect(()=>{
    const down=(event:KeyboardEvent)=>{
      ...
      if(event.key.toLowerCase()==='m'&&!locked){if(conditionAuthoringActive){if(props.roleMeasurementRequest){startedRoleRequestIdRef.current=null;beginRoleMeasurementRequest(props.roleMeasurementRequest,true);}else openConditions();return;}if(buildPlanReady)setTool('draw');else setMessage('Verify the current build method before starting takeoff.');}
  ...
  function changePage(next:number){const sheet=initialSheets.find((entry:any)=>Number(entry.page_number)===Number(next))||null;setPageNumber(next);props.onActiveSheetChange(sheet?String(sheet.id):null);setDraftPoints([]);setDraftScaleRegionId(null);setCalibrationPoints([]);setScaleRegionPoints([]);setPendingScaleCandidate(null);setPendingManualCalibration(null);setEditGeometry(null);editOriginalRef.current=null;setHoverPoint(null);setSelectedMeasurementId(null);setConditionDrawActive(false);startedRoleRequestIdRef.current=null;if(conditionAuthoringActive&&!props.roleMeasurementRequest)setSelectedAssemblyId('');setTool('select');setZoom(1);}
  ...
  return <div className={`${styles.workstation} ${legacyChrome?'':styles.directCanvas}`}>
  <div className={`${styles.workspace} ${!legacyChrome||!sheetsOpen?styles.noSheets:''} ${!legacyChrome||!inspectorOpen?styles.noInspector:''}`}>
    {legacyChrome&&sheetsOpen&&<aside className={styles.sidebar}>
    ...
    {legacyChrome&&!conditionAuthoringActive&&buildPlanWorkbenchOpen&&selectedAssembly&&selectedVersion?<section className={styles.buildPlanWorkspace}>
      ...
      <div className={styles.toolbar}>
        <div className={styles.toolGroup}>
          {legacyChrome&&!sheetsOpen&&<button type="button" className={styles.toolButton} title="Show sheets" onClick={()=>setSheetsOpen(true)}><PanelLeftOpen size={16}/></button>}
        ...
        <div className={styles.zoomGroup}>
          ...
          {legacyChrome&&!inspectorOpen&&<button type="button" className={styles.iconTool} title="Show takeoff inspector" onClick={()=>setInspectorOpen(true)}><PanelRightOpen size={16}/></button>}
        </div>
      </div>
    ...
    {legacyChrome&&inspectorOpen&&<aside className={styles.inspector}>
  ...
  {legacyChrome&&<TakeoffQuantityDock
