...
export function Takeoff3DToolbar({ label, onHome, onTop, issueCount, checksOpen, onToggleChecks, onFocus, canFocus, filtersOpen, onToggleFilters }: {
  ...
  return <div className={styles.toolbar} aria-label="3D view controls">
    ...
    <Button size="xs" variant="ghost" onClick={onFocus} disabled={!canFocus} title="Focus selected Takeoff"><Focus />Focus</Button>
    <Button size="xs" variant="ghost" onClick={onToggleFilters} aria-expanded={filtersOpen}><SlidersHorizontal />Filters</Button>
