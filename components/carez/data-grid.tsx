...
export const CarezDataGrid=React.forwardRef<HTMLDivElement,React.ComponentProps<'div'>&{
  ...
  return <div ref={ref} data-slot="carez-data-grid" aria-busy={loading||undefined} className={cn('min-w-0 overflow-hidden rounded-md border border-border bg-background shadow-none outline-none focus-visible:ring-2 focus-visible:ring-ring/30',className)} {...props}>
    {toolbar?<div data-slot="carez-data-grid-toolbar" className="flex min-h-[var(--density-control-height)] flex-wrap items-center gap-2 border-b border-border px-3 py-1.5">{toolbar}</div>:null}
...
export function CarezDataGridRow({className,selected=false,...props}:React.ComponentProps<'tr'>&{selected?:boolean}){
  return <tr {...props} data-slot="carez-data-grid-row" aria-selected={selected||undefined} className={cn('border-b border-border/70 transition-colors duration-150 hover:bg-muted/35 aria-selected:bg-accent/70 motion-reduce:transition-none',className)}/>;
}
...
export function CarezDataGridHeaderCell({className,numeric=false,resizable=false,sortable=false,sort,...props}:React.ComponentProps<'th'>&{numeric?:boolean;resizable?:boolean;sortable?:boolean;sort?:React.AriaAttributes['aria-sort']}){
  return <th {...props} data-slot="carez-data-grid-header-cell" aria-sort={sort} data-sortable={sortable||undefined} className={cn('h-[var(--density-row-height)] whitespace-nowrap px-2 text-left align-middle text-[11px] font-medium',numeric&&'text-right font-mono tabular-nums',resizable&&'resize-x overflow-hidden',sortable&&'select-none',className)}/>;
}
