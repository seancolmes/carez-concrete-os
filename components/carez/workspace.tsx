...
export function CarezToolbar({className,...props}:React.ComponentProps<'div'>){
  return <div data-slot="carez-toolbar" className={cn('flex min-h-9 items-center gap-1 overflow-x-auto border-b border-border bg-background px-2 py-1',className)} {...props}/>;
}
  ...
  const renderNode=(node:CarezConditionTreeNode,depth:number):React.ReactNode=>{
    ...
    return <React.Fragment key={node.id}>
      <div className={cn('group flex min-h-8 items-center gap-1 border-b border-border/50 pr-1 text-xs',selected?'bg-accent text-accent-foreground':'hover:bg-muted/40',node.disabled&&'opacity-50')} style={{paddingLeft:`${6+depth*14}px`}}>
        ...
        {onVisibilityChange?<Button type="button" variant="ghost" size="icon-xs" className="opacity-60 group-hover:opacity-100" onClick={()=>onVisibilityChange(node,!node.hidden)} aria-label={node.hidden?`Show ${node.label}`:`Hide ${node.label}`}>{node.hidden?<EyeOff/>:<Eye/>}</Button>:null}
      </div>
