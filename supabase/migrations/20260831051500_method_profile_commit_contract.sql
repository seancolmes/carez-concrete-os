-- Carez P1: bind drawing measurements to the exact verified build method.
...
begin
  if new.method_profile_id is null then
    if exists (
      ...
      raise exception 'Verify the build method before creating or recalculating this takeoff.';
    end if;
    return new;
  end if;
  ...
  if exists (
    ...
    raise exception 'Builder or production assumptions changed. Verify a new build-method profile before recalculating this takeoff.';
  end if;
...
before insert or update of method_profile_id,takeoff_set_id,assembly_version_id,variables
...
begin
  ...
  if p_method_inputs is null or jsonb_typeof(p_method_inputs)<>'object' then
    raise exception 'Builder method inputs are required.';
  end if;

  perform 1
  from public.takeoff_sets s
  where s.id=p_takeoff_set_id
  ...
  if not exists (
    ...
    where v.id=p_assembly_version_id
      and v.company_id=v_company
      and v.status='published'
  ) then
    raise exception 'Published assembly version not found.';
  end if;
  ...
  where company_id=v_company
    ...
    and assembly_version_id=p_assembly_version_id;
  ...
  insert into public.takeoff_method_profiles(
    ...
    verification_notes,verified_by,verified_at,created_by
  ) values (
    v_company,p_takeoff_set_id,p_assembly_version_id,v_revision,
    coalesce(nullif(trim(p_name),''),'Verified Build Method'),
    'draft',p_method_inputs,nullif(trim(coalesce(p_verification_notes,'')),''),
    null,null,auth.uid()
...
-- Replace the drawing commit RPC with an optional method-profile argument.
...
create or replace function public.carez_commit_drawing_measurement(
  ...
  p_method_profile_id uuid default null
)
returns uuid
language plpgsql
security invoker
...
begin
  ...
  if exists (
    ...
    if p_method_profile_id is null then
      raise exception 'Verify the build method before starting this takeoff.';
    end if;
    select * into v_profile
    from public.takeoff_method_profiles p
    where p.id=p_method_profile_id
      and p.company_id=v_company_id
      and p.takeoff_set_id=p_takeoff_set_id
      and p.assembly_version_id=p_assembly_version_id
      and p.status='verified';
    ...
    if exists (
      ...
      where v.company_id=v_company_id
        ...
        and (
          not (v_profile.method_inputs ? v.variable_key)
          or v_profile.method_inputs->v.variable_key is distinct from coalesce(p_variables,'{}'::jsonb)->v.variable_key
        )
    ) then
      raise exception 'Current builder assumptions do not match the verified build-method profile.';
    end if;
  ...
  -- Underlying generic commit creates the measurement/output/estimate-item lineage atomically.
  -- The method-profile update remains in this same transaction and is validated by the row trigger.
  ...
  update public.takeoff_measurements
    set sheet_id=p_sheet_id,
        geometry=p_geometry,
        source='drawing',
        scale_region_id=case when p_measurement_type='count' then null else p_scale_region_id end,
        method_profile_id=p_method_profile_id
