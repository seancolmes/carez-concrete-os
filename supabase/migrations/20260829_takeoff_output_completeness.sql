-- A committed takeoff object must contain every component in the selected published assembly exactly once.
...
begin
  ...
  select count(*) into v_expected_count from public.concrete_assembly_components c where c.company_id=v_company_id and c.assembly_version_id=p_assembly_version_id;
  select count(distinct value->>'assembly_component_id') into v_payload_count from jsonb_array_elements(coalesce(p_outputs,'[]'::jsonb));
  ...
  loop
    select * into v_component from public.concrete_assembly_components c
    where c.id=nullif(v_payload->>'assembly_component_id','')::uuid and c.assembly_version_id=p_assembly_version_id and c.company_id=v_company_id;
