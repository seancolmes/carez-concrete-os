...
begin
  ...
  if old.status='verified' then
    if new.status='retired'
       ...
       and new.verification_notes is not distinct from old.verification_notes then return new; end if;
...
create trigger takeoff_method_profiles_guard before update on public.takeoff_method_profiles
...
create trigger takeoff_method_profiles_touch before update on public.takeoff_method_profiles
...
create policy takeoff_method_profiles_select on public.takeoff_method_profiles for select to authenticated
using (company_id=public.get_my_company_id());

drop policy if exists takeoff_method_profiles_insert on public.takeoff_method_profiles;
create policy takeoff_method_profiles_insert on public.takeoff_method_profiles for insert to authenticated
with check (company_id=public.get_my_company_id() and public.get_my_role()<>'employee' and status='draft');

drop policy if exists takeoff_method_profiles_update on public.takeoff_method_profiles;
create policy takeoff_method_profiles_update on public.takeoff_method_profiles for update to authenticated
...
create policy takeoff_method_profiles_delete on public.takeoff_method_profiles for delete to authenticated
...
begin
  ...
  ) then raise exception 'Verify every required builder/production assumption before starting takeoff.'; end if;
  perform set_config('carez.method_profile_verify','1',true);
  update public.takeoff_method_profiles set status='verified',verified_by=auth.uid(),verified_at=now()
