...
create trigger takeoff_measurements_method_profile_guard before insert or update of method_profile_id,takeoff_set_id,assembly_version_id,variables
on public.takeoff_measurements for each row execute function public.carez_validate_measurement_method_profile();

create or replace function public.carez_create_verified_takeoff_method_profile(
  p_takeoff_set_id uuid,p_assembly_version_id uuid,p_name text,p_method_inputs jsonb,p_verification_notes text default null
)
returns uuid language plpgsql security invoker set search_path=public as $$
declare v_company uuid:=public.get_my_company_id(); v_profile_id uuid; v_revision integer;
begin
  if v_company is null or public.get_my_role()='employee' then raise exception 'Owner access required.'; end if;
  if p_method_inputs is null or jsonb_typeof(p_method_inputs)<>'object' then raise exception 'Builder method inputs are required.'; end if;
  perform 1 from public.takeoff_sets s where s.id=p_takeoff_set_id and s.company_id=v_company and s.status='active' for update;
  if not found then raise exception 'Active takeoff set not found.'; end if;
  if not exists (select 1 from public.concrete_assembly_versions v where v.id=p_assembly_version_id and v.company_id=v_company and v.status='published') then raise exception 'Published assembly version not found.'; end if;
  if exists (
    select 1 from public.concrete_assembly_variables v
    where v.company_id=v_company and v.assembly_version_id=p_assembly_version_id and v.requires_verification
      and v.input_role in ('method_decision','production_assumption','commercial_assumption')
      and public.carez_method_activation_matches(v.activation_rule,p_method_inputs)
      and not (p_method_inputs ? v.variable_key)
  ) then raise exception 'Verify every active builder/production assumption before starting takeoff.'; end if;
  select coalesce(max(revision_no),0)+1 into v_revision from public.takeoff_method_profiles
  where company_id=v_company and takeoff_set_id=p_takeoff_set_id and assembly_version_id=p_assembly_version_id;
  insert into public.takeoff_method_profiles(company_id,takeoff_set_id,assembly_version_id,revision_no,name,status,method_inputs,verification_notes,verified_by,verified_at,created_by)
  values(v_company,p_takeoff_set_id,p_assembly_version_id,v_revision,coalesce(nullif(trim(p_name),''),'Verified Build Method'),'draft',p_method_inputs,nullif(trim(coalesce(p_verification_notes,'')),''),null,null,auth.uid())
  returning id into v_profile_id;
  perform set_config('carez.method_profile_verify','1',true);
  update public.takeoff_method_profiles set status='verified',verified_by=auth.uid(),verified_at=now() where id=v_profile_id and company_id=v_company;
  return v_profile_id;
end;
$$;
revoke all on function public.carez_create_verified_takeoff_method_profile(uuid,uuid,text,jsonb,text) from public,anon;
grant execute on function public.carez_create_verified_takeoff_method_profile(uuid,uuid,text,jsonb,text) to authenticated,service_role;

drop function if exists public.carez_commit_drawing_measurement(uuid,uuid,uuid,uuid,text,text,text,text,numeric,text,jsonb,text,jsonb,jsonb,uuid);
...
returns uuid language plpgsql security invoker set search_path=public as $$
declare v_company_id uuid:=public.get_my_company_id(); v_measurement_id uuid; v_scale_region public.takeoff_scale_regions%rowtype; v_profile public.takeoff_method_profiles%rowtype;
begin
