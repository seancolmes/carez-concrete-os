-- Carez P1 correction: allow the internal generic measurement commit to create
-- a transient unbound row inside the drawing RPC transaction, then bind the
-- verified method profile before commit. Direct/unverified callers remain blocked.
...
begin
  if new.method_profile_id is null then
    if coalesce(current_setting('carez.allow_unbound_method_profile_insert',true),'')='1' then
      return new;
    end if;
...
declare
  ...
  v_profile public.takeoff_method_profiles%rowtype;
  v_requires_profile boolean:=false;
begin
  ...
  if v_requires_profile then
    ...
    where p.id=p_method_profile_id
      ...
      and p.status='verified';
    if not found then
      raise exception 'Verified build-method profile not found for this takeoff.';
    end if;
  ...
  if p_measurement_type<>'count' then
    if p_scale_region_id is null then
      select id into p_scale_region_id
    ...
    select * into v_scale_region
    from public.takeoff_scale_regions
    where id=p_scale_region_id
      and company_id=v_company_id
      and sheet_id=p_sheet_id;
    if v_scale_region.id is null then
      raise exception 'An accepted scale region is required for LF or SF takeoff.';
    end if;
    if not public.carez_geometry_within_scale_bounds(p_geometry,v_scale_region.region_bounds) then
      raise exception 'Takeoff geometry must stay inside one accepted scale region.';
    end if;
  end if;

  if v_requires_profile then
    perform set_config('carez.allow_unbound_method_profile_insert','1',true);
  end if;
  ...
  if v_requires_profile then
    perform set_config('carez.allow_unbound_method_profile_insert','0',true);
  end if;
