-- Reconcile environments that received the custom assembly tables but missed the
-- company-owned blank assembly creation RPC. This is idempotent and matches the
-- canonical custom assembly authoring foundation contract.

create or replace function public.carez_create_custom_assembly(
  p_code text,
  p_name text,
  p_category text,
  p_primary_measurement text,
...
begin
  ...
  if nullif(trim(p_code),'') is null
    ...
    raise exception 'Assembly code, name, and category are required.';
  end if;

  if v_measurement not in ('LF','SF','EA','CY') then
    raise exception 'Primary measurement must be LF, SF, EA, or CY.';
  end if;

  if p_folder_id is not null and not exists(
    select 1
    from public.concrete_assembly_folders f
    where f.id=p_folder_id
      and f.company_id=v_company
      and f.active
  ) then
    raise exception 'Assembly folder not found.';
  end if;

  insert into public.concrete_assemblies(
    company_id,folder_id,code,name,category,primary_measurement,description,created_by
  ) values(
    v_company,
    p_folder_id,
    trim(p_code),
  ...
  return jsonb_build_object(
    'assembly_id',v_assembly,
    'assembly_version_id',v_version,
    'version_no',1,
    'status','draft'
  );
end;
