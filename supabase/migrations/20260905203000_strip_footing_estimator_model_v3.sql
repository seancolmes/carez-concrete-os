...
select
  ...
  $json$[
    ...
    {"key":"reinforcing","label":"Reinforcing","repeatable":true,"default_enabled":false,"input_schema":[
      {"key":"kind","label":"Reinforcing location / pattern","value_type":"select","options":["bottom_longitudinal","top_longitudinal","transverse","dowel","stirrup","custom"]},
      ...
      {"key":"bar_count","label":"Bars total","value_type":"integer","unit":"EA","minimum":1},
      {"key":"spacing_in","label":"Spacing / centers","value_type":"number","unit":"IN","minimum":0.000001},
      {"key":"pieces_per_location","label":"Pieces per location","value_type":"integer","unit":"EA","minimum":1},
      {"key":"piece_length_ft","label":"Piece length","value_type":"number","unit":"FT","minimum":0.000001},
      ...
      {"key":"stock_length_ft","label":"Stock length","value_type":"number","unit":"FT","minimum":0.000001},
      {"key":"lap_length_in","label":"Lap length","value_type":"number","unit":"IN","minimum":0},
      {"key":"custom_total_length_ft","label":"Custom total length","value_type":"number","unit":"FT","minimum":0.000001},
    ...
    {"key":"excavation_backfill","label":"Excavation / backfill","repeatable":false,"default_enabled":false,"input_schema":[
      ...
      {"key":"backfill_pct","label":"Backfill share of remaining void","value_type":"number","unit":"%","minimum":0,"maximum":100},
...
where archetype.code='strip_wall_footing'
  and not exists (
    ...
    where existing.archetype_id=archetype.id and existing.version_no=3
  );
...
begin
  ...
  where archetype_id=v_archetype.id and version_no=3 and status='published'
  limit 1;
  ...
  where template.company_id=v_company and template.active and template.archetype_id=v_archetype.id
    and template_version.status='published' and template_version.archetype_version_id=v_archetype_version.id
  ...
  select assembly.id into v_assembly_id from public.concrete_assemblies assembly
  where assembly.company_id=v_company and assembly.code=v_assembly_code limit 1;
  ...
  insert into public.concrete_assembly_versions(
    ...
    'Generated by carez_ensure_strip_footing_v3_template for Issue #55.',
  ...
  select
    ...
    coalesce((output->>'estimate_visible')::boolean,true),
    jsonb_build_object('mode','condition_compatibility','condition_output_key',output->>'key'),ordinality::integer*10
  ...
  select template.id into v_template_id from public.company_condition_templates template
  ...
  insert into public.company_condition_template_versions(
    ...
    'Issue #55 Strip / Wall Footing v3 estimator model. Project values remain estimator-confirmed.',auth.uid()
...
comment on function public.carez_ensure_strip_footing_v3_template() is
  'Creates immutable Strip / Wall Footing v3 company template and compatibility projection while preserving v1/v2 lineage.';
