-- Carez Assembly & Resource Engine: evolve verified build-method profiles into
-- a backward-compatible Project Scope Variant model.
...
begin
  ...
  if p_inputs is null or jsonb_typeof(p_inputs)<>'object' then
    raise exception 'Project scope variant inputs must be an object.';
  end if;
  ...
  if not exists (
    ...
    raise exception 'Published Scope Recipe version not found.';
  end if;
  ...
  if exists (
    ...
    raise exception 'Resolve every required active recipe variable before saving the Project Scope Variant.';
  end if;
...
begin
  if new.method_profile_id is null then
    ...
    if exists (
      ...
      raise exception 'Verify the build method or select a Project Scope Variant before creating or recalculating this takeoff.';
    end if;
    return new;
  end if;
  ...
  if not found then
    raise exception 'Takeoff profile/variant must be verified and match the same takeoff set and Scope Recipe version.';
  end if;
