-- Carez OS: acceptance becomes an operational handoff.
...
begin
  ...
  for m in
    select x.* from (
      ...
      join public.estimate_items i
        on i.company_id=tm.company_id and i.source_takeoff_measurement_id=tm.id and i.estimate_id=e.id
  ...
  loop
    ...
    if v_concrete_cy>0 then
      ...
      if v_pour is null then
        ...
        join public.project_budget_sections pbs on pbs.budget_id=pb.id and pbs.company_id=pb.company_id
        where pb.project_id=p_project_id and pb.estimate_id=e.id and pb.budget_type='original' and pb.status='active'
          and pbs.source_estimate_section_id=m.estimate_section_id
        order by pbs.sort_order limit 1;
    ...
    -- Structural and ROW assemblies get a real pre-pour gate. Generic private flatwork stays judgment-based.
    ...
    -- Material/equipment/subcontractor estimate facts become operation-level readiness requirements.
    for r in
      select
  ...
  left join public.work_package_resource_requirements rr on rr.work_package_operation_id=wo.id and rr.company_id=wo.company_id
  left join public.project_inspections pi on pi.required_for_operation_id=wo.id and pi.company_id=wo.company_id
  left join public.pour_plans pp on pp.source_work_package_id=wp.id and pp.company_id=wp.company_id
  where wp.company_id=e.company_id and wp.project_id=p_project_id and wp.source_estimate_id=e.id and wp.source_type='takeoff';
...
-- Award remains the single project/budget boundary, but now preserves takeoff lineage and generates field operations.
