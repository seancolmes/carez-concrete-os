-- Carez P1.4 prerequisite: reconcile the accepted Proposal conversion schema into the
-- current staging migration chain without changing Proposal product behavior.
-- Historical behavior source: commit 1d6490c53151cfb964d6dba213959bf5fb88cdbe
-- (20260827_remaining_os_modules.sql + 20260828_customer_proposal_conversion.sql).
--
-- proposal_presentations already exists in the current staging baseline, so this
-- migration restores only the missing companion relations/contracts and reattaches
-- its historical token relationship without rewriting existing presentation rows.
...
create table if not exists public.proposal_clarifications (
  ...
  category text not null
    check (category in ('inclusion','exclusion','assumption','allowance','qualification')),
...
create table if not exists public.proposal_engagement_events (
  ...
  event_type text not null
    check (event_type in ('view','question','change_request','option_interest','decline','accept')),
  customer_name text,
  customer_email text,
  customer_message text,
  decline_reason text,
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conrelid='public.proposal_presentations'::regclass
      and contype='f'
      and confrelid='public.proposal_access_tokens'::regclass
      and pg_get_constraintdef(oid) like 'FOREIGN KEY (proposal_access_token_id)%'
  ) then
    alter table public.proposal_presentations
      add constraint proposal_presentations_access_token_fk
      foreign key (proposal_access_token_id)
      references public.proposal_access_tokens(id)
      on delete cascade
      not valid;
  end if;
end;
