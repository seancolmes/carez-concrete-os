-- Takeoff plan documents may only reference a PDF uploaded under this company's plan prefix.
...
begin
  ...
  if length(trim(p_storage_path)) > 500 or length(trim(p_source_filename)) > 255 then
    raise exception 'Plan file name or path is too long.';
  end if;
  ...
  if position(v_expected_prefix in trim(p_storage_path)) <> 1 then
    raise exception 'Plan storage path does not belong to this company.';
  end if;
