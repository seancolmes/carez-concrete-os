-- Preserve explicit missing-input readiness on nested assembly branches even when
-- the unresolved activation choice means the branch cannot yet be active.
...
begin
  ...
  for v_payload in select value from jsonb_array_elements(p_outputs) loop
    ...
    if v_active and v_output.id is not null and v_output.pricing_status='manual_override' then
      ...
      -- missing_input is a readiness hold, not a priced output. Preserve the
      -- engine's explanation even though the unresolved branch is inactive.
      v_cost_source:=case
        when (v_payload->>'pricing_status')='missing_input' then nullif(v_payload->>'cost_source','')
        when v_active then nullif(v_payload->>'cost_source','')
        else null
      end;
      v_status:=case
        when (v_payload->>'pricing_status')='missing_input' then 'missing_input'
    ...
    if v_output.id is null then
      insert into public.takeoff_measurement_outputs(
        company_id,measurement_id,assembly_component_id,component_key,label,estimate_item_type,cost_code_id,catalog_item_id,
        production_task_id,production_quantity,production_unit,estimated_man_hours,baseline_man_hours_per_unit,baseline_source,
        unit_cost,cost_source,direct_cost,pricing_status,is_active,resource_behavior,estimate_visible,formula_trace
      ) values(
        v_company,v_measurement.id,v_component.id,v_path,v_component.label,v_component.estimate_item_type,v_component.cost_code_id,
        v_component.catalog_item_id,v_component.production_task_id,v_qty,v_component.output_unit,v_hours,
        nullif(v_payload->>'baseline_man_hours_per_unit','')::numeric,v_component.baseline_source,v_unit_cost,v_cost_source,v_direct,
        v_status,v_active,v_behavior,v_visible,coalesce(v_payload->'formula_trace','{}'::jsonb)
    ...
    if not v_active or not v_visible then
      ...
      if v_item_id is null then
        insert into public.estimate_items(
          ...
          quantity,unit,unit_cost,direct_cost,regular_hours,overtime_hours,notes,sort_order,source_takeoff_output_id,
          source_takeoff_measurement_id,source_assembly_version_id,production_task_id,production_quantity,production_unit,
          baseline_man_hours_per_unit,baseline_source
        ) values(
          v_company,v_measurement.estimate_id,v_measurement.estimate_section_id,v_component.estimate_item_type,v_component.cost_code_id,
          v_component.catalog_item_id,v_component.labor_task,case when v_component.estimate_item_type='labor' then v_measurement.risk_class_code else null end,
          ...
          v_sort,v_output.id,v_measurement.id,v_measurement.assembly_version_id,v_component.production_task_id,v_qty,
          v_component.output_unit,nullif(v_payload->>'baseline_man_hours_per_unit','')::numeric,v_component.baseline_source
