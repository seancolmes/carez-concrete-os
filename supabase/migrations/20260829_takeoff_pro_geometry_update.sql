-- Carez OS: edit an existing drawing measurement without breaking its estimate-line lineage.
...
begin
  ...
  loop
    ...
    set production_quantity=greatest(coalesce((v_payload->>'production_quantity')::numeric,0),0),
        production_unit=v_component.output_unit,
        estimated_man_hours=greatest(coalesce((v_payload->>'estimated_man_hours')::numeric,0),0),
        ...
        unit_cost=greatest(coalesce((v_payload->>'unit_cost')::numeric,0),0),
        cost_source=nullif(v_payload->>'cost_source',''),
        direct_cost=greatest(coalesce((v_payload->>'direct_cost')::numeric,0),0),
        pricing_status=coalesce(v_payload->>'pricing_status','missing_price'),
    ...
    set quantity=case
          when v_component.estimate_item_type='labor' then greatest(coalesce((v_payload->>'estimated_man_hours')::numeric,0),0)
          else greatest(coalesce((v_payload->>'production_quantity')::numeric,0),0)
        end,
        unit=case when v_component.estimate_item_type='labor' then 'HR' else v_component.output_unit end,
        unit_cost=greatest(coalesce((v_payload->>'unit_cost')::numeric,0),0),
        direct_cost=greatest(coalesce((v_payload->>'direct_cost')::numeric,0),0),
        regular_hours=case when v_component.estimate_item_type='labor' then greatest(coalesce((v_payload->>'estimated_man_hours')::numeric,0),0) else 0 end,
        overtime_hours=0,
        production_task_id=v_component.production_task_id,
        production_quantity=greatest(coalesce((v_payload->>'production_quantity')::numeric,0),0),
        production_unit=v_component.output_unit,
        baseline_man_hours_per_unit=nullif(v_payload->>'baseline_man_hours_per_unit','')::numeric,
        baseline_source=v_component.baseline_source,
        updated_at=now()
