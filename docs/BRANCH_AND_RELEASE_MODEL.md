...
1. Discuss the idea in the owning Carez chat.
2. Promote approved behavior into the applicable canonical GitHub spec/issue when material.
3. Implement the smallest coherent change directly on `staging` when the available tooling and risk level make that safe.
4. Run typecheck/tests/build.
5. Wait for the stable staging deployment to update.
6. Nik tests the same staging URL.
7. Record browser acceptance and update `CURRENT_STATE.md` when verified state changes.
8. Promote `staging` to `main` only as an explicit production release after acceptance.
