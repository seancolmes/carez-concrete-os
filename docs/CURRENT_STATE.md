...
- `app/globals.css` is now the Carez semantic shadcn theme: light neutral application surfaces, restrained Carez blue interaction color, semantic success/warning/destructive states, and permanent dark sidebar tokens;
...
- `components/AppShell.tsx` was replaced with the actual shadcn `Sidebar` system (`SidebarProvider`, `Sidebar`, `SidebarContent`, groups/menus, `SidebarFooter`, `SidebarRail`, `SidebarInset`, `SidebarTrigger`), plus shadcn Breadcrumb, Command, Button, Separator, and Tooltip behavior;
...
- Ctrl/Cmd+K now opens a shadcn Command route palette;
- Login is rebuilt with shadcn Card/Input/Label/Button;
- Dashboard / Today is rebuilt with shadcn Card, Badge, Table, Empty, Progress, and Button primitives while preserving its existing management/readiness/cash logic;
...
Automated validation is required on the final staging checkpoint: Typecheck, Domain tests, and Next.js Build must pass before rendered QA begins. Vercel must report the matching staging SHA READY on the stable staging alias.

This is an implementation checkpoint, not rendered visual acceptance. Authenticated cross-route QA is still required for contrast, spacing, overflow, mobile behavior, Takeoff workstation chrome, and any legacy-markup selectors still served through the temporary compatibility bridge.
...
1. Complete automated validation and stable-staging deployment for the shadcn redesign checkpoint under Issue #44.
2. Browser-QA Dashboard / Today, Projects, Takeoff list, Takeoff drawing workspace, Estimates, Owner Reports, and representative CRM/Field/Finance/Settings routes on the single stable staging URL.
3. Correct route-specific visual/interaction regressions under Issue #44; continue replacing temporary compatibility selectors with literal shadcn components.
4. Re-verify and close Issue #35 when Takeoff pane/header/readability behavior is confirmed under the shadcn shell.
5. Continue the accepted Concrete Condition / derived 2D+3D Takeoff implementation sequence and remaining controlled acceptance gates from `ROADMAP.md`.
