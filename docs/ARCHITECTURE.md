...
- The estimator works through an ADR-020 integrated workstation: a stable-width independently collapsible Plans/Conditions/Zones context pane; a dominant drawing surface; one stable-width independently collapsible docked Condition Properties pane; and the worksheet. The normal docked side panes are not horizontally drag-resizable. A future explicit floating-properties mode may be draggable/resizable without changing the docked contract.
...
- Window/pane layout changes preserve sheet, viewport, zoom/pan, calibration, selection, active tool, and unsaved property state.
...
```text
[ compact application menubar: identity | inline global categories | search/actions/account ]
...
The permanent global desktop left app rail and the superseded stacked second global category row are retired from the accepted target. Contextual module panes remain part of the current workflow and are distinct from global navigation.

The compact application menubar owns authentic Carez/company identity, inline global categories, company/project context where useful, global command/search, notifications, restrained quick-create behavior where justified, and account controls. Category triggers open compact keyboard-accessible anchored menus beneath the same menubar. Mobile may collapse global navigation into a Sheet/drawer rather than reproducing the desktop menubar literally.

Visual direction is governed by ADR-025 as revised by FINAL-2026-09-26: Steam Sleek Light Blue Steel. Light concrete-white workspaces, pale steel navigation, compact technical controls, thin borders, and accessible azure/cyan interaction replace the earlier dark/Indigo Harbor palettes. Source-owned shadcn/ui primitives and the shared Carez component pack remain one component system. Current Command Rail / Domain Deck / contextual Command Bar ownership supersedes older shell arrangements; presentation never owns quantity or financial authority.

Use the Estimating EDGE interaction pattern only as a benchmark for clear condition organization, professional screen division, and estimator workflow—not as a visual theme. Prefer crisp separators, tabs, dropdowns, restrained semantic color, readable grids, compact tables, sheets/popovers/dialogs, contextual panes, and a small number of predictable governed overlays. Normal docked Takeoff side panes follow ADR-020 stable-width collapse/restore; any future explicit floating-properties mode may support drag/resize without turning the application into uncontrolled overlapping windows. Avoid tiny text, cramped tool chrome, generic SaaS card walls, giant rounded cards, glassmorphism, decorative UI, excessive unused whitespace, perpetual decorative motion, and uncontrolled overlapping dialogs.
...
- Render Today beneath the compact ADR-016 application menubar; it does not use a permanent global left rail or a second persistent global navigation row.
